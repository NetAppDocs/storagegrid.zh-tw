...
雲端儲存池允許您將物件儲存在 StorageGRID 系統之外。例如，您可能想要將不常存取的物件移至成本較低的雲端儲存，例如 Amazon S3 Glacier、S3 Glacier Deep Archive、Google Cloud 或 Microsoft Azure Blob Storage 中的封存存取層。或者，您可能想要維護 StorageGRID 物件的雲端備份，以便在儲存設備 Volume 或儲存節點故障時恢復遺失的資料。
...
* 搜尋整合服務將物件中繼資料傳送到外部 Elasticsearch 服務，從而可以使用第三方工具搜尋、視覺化和分析中繼資料。
...
與手動分層解決方案不同， FabricPool透過自動化資料分層來降低儲存成本，從而降低整體擁有成本。它透過分層到公有雲和私有雲（包括StorageGRID）來提供雲端經濟的優勢。
