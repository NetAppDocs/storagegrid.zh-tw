...
image:storagegrid_system_diagram.png["映像與多個用戶端"]
...
* 支援基於軟體或硬體的部署。您可以將 StorageGRID 部署在以下任何平台上：
+
** 在VMware中執行的虛擬機器。
...
* 聯合身分管理。與 Active Directory、OpenLDAP 或 Oracle Directory Service 整合，用於使用者身分驗證。支援使用安全性斷言標記語言 2.0 （SAML 2.0） 或 OpenID Connect （OIDC） 的單一登入 （SSO），以在 StorageGRID 和外部身分供應商（例如 Active Directory Federation Services （AD FS））之間交換驗證和授權資料。
