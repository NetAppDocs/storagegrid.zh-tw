...
image:replication_data_flow.png["顯示相關文字中所述程序的圖表"]
...
image:erasure_coding_data_flow.png["銷毀編碼資料流程"]
...
如果 ILM 規則的內容放置說明要求將物件資料的複寫複本儲存在雲端儲存池中，物件資料將複製到為雲端儲存池指定的外部 S3 儲存貯體或 Azure Blob Storage Container。

ILM引擎是LDR服務的一項元件、而Data Mover服務則可控制物件移至雲端儲存池的動作。

image:cloud_storage_pool_data_flow.png["雲端儲存資源池資料流"]
