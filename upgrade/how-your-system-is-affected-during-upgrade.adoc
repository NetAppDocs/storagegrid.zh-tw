...
StorageGRID 12.1 升級期間：

* 所有 StorageGRID 應用裝置節點會自動升級至 StorageGRID Appliance Installer 韌體版本 12.1。
* SG6060 和 SGF6024 設備自動升級到 BIOS 韌體版本 3B08.EX 和BMC韌體版本 4.01.07。
* SG100 和 SG1000 裝置自動升級至 BIOS 韌體版本 3B13.EC 和BMC韌體版本 4.75.07。
* SGF6112、SG6160、SG110 和 SG1100 應用裝置將自動升級至 BIOS 韌體版本 3A14.QD 和 BMC 韌體版本 3.21.07。
...
例如，當服務停止時，您可能會看到 *Unable to communicate with node* 警示；或者，當某些節點已升級至 StorageGRID 12.0 或更高版本，而其他節點仍在執行舊版 StorageGRID 時，您可能會看到 *Cassandra communication error* 警示。通常、這些警示會在升級完成後消失。

在升級至 StorageGRID 12.0 或更新版本期間，若儲存節點停止運作，可能會觸發 *ILM placement unachievable* 警示。此警示可能在升級完成後持續 1 天。
...
NOTE: 組態變更限制因版本而異。如果您要升級至其他 StorageGRID 版本，請參閱該版本升級說明中的受限變更清單。
...
* 升級期間，請勿管理 HA 群組、VLAN 介面或負載平衡器端點。
* 升級完成前請勿刪除任何高可用性 (HA) 組。否則，其他 HA 群組中的虛擬 IP 位址可能無法存取。
