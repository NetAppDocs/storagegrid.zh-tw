...
https://learn.microsoft.com/en-us/troubleshoot/windows-server/active-directory/support-boundary-high-accuracy-time["高精度時間的支援邊界"^]
...
（僅限 VMware）對 VMware 執行額外檢查，例如確保虛擬機器管理程式使用與虛擬機器相同的 NTP 來源，並使用 VMTools 停用虛擬機器管理程式和StorageGRID虛擬機器之間的時間同步。

.步驟
. 在「*伺服器1*到*伺服器4*」文字方塊中、指定至少四部NTP伺服器的IPV4位址。
. 如有必要、請選取最後一個項目旁的加號、以新增其他伺服器項目。
+
image:8_gmi_installer_ntp_page.gif["此影像由周邊文字說明。"]
