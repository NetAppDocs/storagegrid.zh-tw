...
.步驟
. 在 *Provisioning passphrase* 中，輸入對 StorageGRID 系統的網格拓撲進行變更所需的 Provisioning passphrase。
+
將資源配置通關密碼儲存在安全的地方。
...
+
image:10_gmi_installer_passwords_page.gif["此影像由周邊文字說明。"]
...
NOTE: 在「摘要」頁面選擇 *Install* 後，系統會提示您下載復原套件檔案(`sgws-recovery-package-id-revision.zip`。您必須link:../maintain/downloading-recovery-package.html["下載此檔案"]完成安裝。存取系統所需的密碼儲存在 `Passwords.txt`檔案中，該檔案包含於復原套件檔案內。
