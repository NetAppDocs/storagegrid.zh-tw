...
. 查看* Summary（摘要）*頁面。
+
image:11_gmi_installer_summary_page.gif["Grid Manager安裝程式摘要頁面"]

. 確認所有網格組態資訊均正確無誤。使用「摘要」頁面上的「修改」連結、即可返回並修正任何錯誤。
. 選擇 *Install*。
+

NOTE: 如果某個節點設定為使用用戶端網路，則當您選擇 *Install* 時，該節點的預設閘道將從 Grid Network 切換到 Client Network。如果連線中斷，您必須確保透過可存取的子網路存取主要管理節點。如需詳細資訊、請參閱 link:../network/index.html["網路準則"]。
...
. 選取「*我已成功下載並驗證 Recovery Package 檔案*」核取方塊，然後選取「*下一步*」。
+
如果安裝仍在進行中、則會顯示狀態頁面。此頁面會指出每個網格節點的安裝進度。

+
image:12_gmi_installer_status_page.gif["此影像由周邊文字說明。"]
