...
summary: 如果您的租用戶帳戶在建立時被指派了 Use grid federation connection 權限，則可以使用跨網格複製功能，在租用戶來源網格的儲存桶和租用戶目標網格的儲存桶之間自動複製物件。跨網格複製可以單向或雙向進行。 
---
...
+
image:grid-federation-cross-grid-replication-client-deletes.png["顯示在兩個網格上複寫用戶端刪除的影像"]
