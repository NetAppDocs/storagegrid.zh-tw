...
.關於這項工作
您可以將通知組態 XML 與來源貯體建立關聯、以設定事件通知。通知組態 XML 遵循 S3 慣例來設定貯體通知，目的地 Amazon SNS 主題， Kafka 主題或 Webhook 端點則指定為端點的 URN 。
...
* 設定事件通知之後，每當來源儲存區中的物件發生指定事件時，就會產生通知，並傳送至 Amazon SNS 主題， Kafka 主題或用作目的地的 Webhook 端點。
...
.. 確認已將通知傳送至目的地 Amazon SNS 主題， Kafka 主題或 Webhook 端點。
+
