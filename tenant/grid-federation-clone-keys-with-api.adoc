...
+
image:grid-federation-post-current-user-replicate.png["用於複寫自己存取金鑰的後端點"]
...
+
image:grid-federation-clone-access-key.png["租戶管理器 API 輸入值以複製存取金鑰"]
...
+
image:grid-federation-post-other-user.png["用於複寫其他使用者存取金鑰的 POST 端點"]
