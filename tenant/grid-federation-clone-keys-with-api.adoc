...
summary: 如果您的 StorageGRID 租用戶帳戶具有使用網格聯合連線權限，則可以使用租用戶管理 API 手動複製 S3 存取金鑰。 
---
...
+
image:grid_federation_post_current_user_replicate.png["用於複寫自己存取金鑰的後端點"]
...
+
image:grid_federation_clone_access_key.png["租戶管理器 API 輸入值以複製存取金鑰"]
...
+
image:grid_federation_post_other_user.png["用於複寫其他使用者存取金鑰的 POST 端點"]
