...
* 您正在將資料儲存在版本化的儲存桶中。由於意外漏洞，導致在時間 _T1_ 和 _T2_ 之間擷取了許多不必要的物件。您可以為 _Exclude_ 時間值 _T1_ 和 _T2_ 建立分支儲存桶，並將用戶端操作重新導向至該分支儲存桶。如此一來，用戶端只會看到在 _T1_ 之前和 _T2_ 之後擷取的物件。




[[operations-on-objects-in-branch-buckets]]
[[operations-in-branch-buckets]]
== 對分支儲存桶中的物件進行操作
...
另請參閱 link:../ilm/how-objects-and-delete-markers-are-deleted.html#delete-s3-versioned-objects["如何刪除S3版本控制物件"]。
