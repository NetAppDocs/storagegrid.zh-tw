...
* 只有當來源桶或目的地桶啟用了 S3 物件鎖定功能時，才支援 Amazon S3 或 StorageGRID S3 儲存桶的複寫。物件鎖定標頭不會複寫到目的地物件。
* 如果您在包含物件的貯體上啟用 CloudMirror 複寫、則會複寫新增至該貯體的物件、但不會複寫該貯體中的現有物件。您必須更新現有物件、才能觸發複寫。
* 如果在設定 CloudMirror 複寫之後對目的地儲存桶啟用 S3 Object Lock，則可能需要link:../tenant/editing-platform-services-endpoint.html["重新儲存端點"]以允許複寫繼續。
