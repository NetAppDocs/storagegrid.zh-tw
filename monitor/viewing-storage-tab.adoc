...
「儲存」標籤會顯示所有節點、每個站台以及整個網格的資訊。此索引標籤包含以下資訊：

儲存使用的圖表:: 對於儲存節點、每個站台及整個網格、「儲存」索引標籤會顯示物件資料和物件中繼資料一段時間以來使用的儲存容量圖表。



NOTE: 當節點未連線至網格時、例如在升級期間或中斷連線狀態時、某些度量可能無法使用或排除在站台和網格總計之外。節點重新連線至網格之後、請等待數分鐘、讓值穩定下來。

image:nodes_storage_node_storage_tab.png["節點 > 儲存節點 > 儲存索引標籤"]
...
image:nodes_page_storage_tables.png["節點頁面儲存表格"]
