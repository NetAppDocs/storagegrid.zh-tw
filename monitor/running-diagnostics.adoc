...
* image:icon_alert_green_checkmark.png["圖示警示綠色核取記號"] *Normal*：所有數值均在正常範圍內。
* image:icon_alert_yellow_minor.png["圖示警示黃色次要"] *Attention*：一個或多個數值超出正常範圍。
* image:icon_alert_red_critical.png["圖示警示紅色嚴重"] *Caution*：其中一個或多個數值明顯超出正常範圍。
...
+
image:support_diagnostics_page.png["支援診斷頁面"]
...
+
image:support_diagnostics_cpu_utilization.png["診斷狀態值範例"]
...
+
image:grafana_dashboard_nodes.png["Grafana儀表板節點"]

. *選用*：若要查看 Prometheus 表示式隨時間變化的圖表，請選擇 *View in Prometheus*。
+
隨即顯示診斷所使用之運算式的Prometheus圖表。

+
image:support_diagnostics_prometheus_png.png["支援診斷Prometheus頁面"]
