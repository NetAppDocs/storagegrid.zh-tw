...
* stream-access.log：流區塊的 TCP/TLS 流（第 4 階層）Proxy 工作階段。
+
*注意*：這不是 HTTP 請求日誌。
...
存取日誌用於記錄透過 S3（全域命名空間/聯盟 URL）路由的網格聯盟流量。
...
| system-bucket.log  a| 
`update-system-bucket-configuration.log`：系統儲存桶日誌。
...
`update-cors-configurations.log`：追蹤與 TLS/密碼和 CORS 變更相關的記錄。
