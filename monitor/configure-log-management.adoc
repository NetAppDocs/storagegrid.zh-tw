...
[role="lead"]
根據需要設定稽核等級、協定標頭、稽核訊息和日誌的位置以及日誌保留。

所有StorageGRID節點都會產生稽核訊息和日誌來追蹤系統活動和事件。審計訊息和日誌是監控和故障排除的重要工具。

您也可以link:../monitor/considerations-for-external-syslog-server.html["設定外部系統日誌伺服器"]選擇將稽核資訊遠端儲存。使用外部伺服器可以最大限度地降低稽核訊息記錄的效能影響，同時又不降低稽核資料的完整性。如果您擁有大型網格、使用多種類型的 S3 應用程式或希望保留所有稽核資料，則外部 syslog 伺服器尤其有用。
...
+
當 StorageGRID 在請求中發現 HTTP 標頭時，它會將這些標頭包含在稽核訊息的 HTRH 欄位中。
...
預設情況下，稽核訊息和日誌會儲存在產生它們的節點上。StorageGRID 會定期輪替並最終刪除稽核訊息和日誌，以防止佔用過多磁碟空間。若要將稽核訊息和部分日誌儲存到外部，<<use-external-syslog-server,使用外部系統日誌伺服器>>。
...
+
如果選擇一個值，StorageGRID 會將所選值套用至所有此類訊息。如果使用固定值置換嚴重性設定，則會遺失有關不同嚴重性的詳細資訊。
...
在使用外部 syslog 伺服器之前，請要求網格中的所有節點向該外部 syslog 伺服器傳送測試訊息。這些測試訊息有助於您在將資料傳送至外部 syslog 伺服器之前，驗證整個記錄收集基礎架構。
...
您可以透過建立 ILM 規則或設定 S3 儲存桶生命週期原則來管理 StorageGRID 保留記錄的時間。

[role="tabbed-block"]
====
.ILM 規則
--
為儲存桶建立 ILM 規則，並將此規則新增至作用中的 ILM 原則。

.步驟
. 選擇* ILM *>* Rules *。
. 選擇 *Create*。link:../ilm/create-ilm-rule-enter-details.html["步驟 1 （輸入詳細資料）"] 「建立 ILM 規則精靈」隨即出現：
+
.. 請為該規則輸入唯一名稱。例如：ILM Rule for archived logs。
.. 選擇日誌歸檔 S3 租戶帳戶。
.. 指定日誌歸檔的 S3 儲存桶。


. 繼續link:../ilm/create-ilm-rule-define-placements.html["步驟 2 （定義放置位置）"]並定義歸檔日誌規則的存放位置。例如，ILM 規則在第一年於站台 1 和站台 2 各儲存一份複本。一年後，產生一份 2+1 的銷除編碼複本，並僅儲存在一個站台。
. 建立規則後，將其新增至作用中的 ILM 原則。


--
.S3 bucket 生命週期
--
在日誌歸檔 S3 儲存桶上設定 S3 儲存桶生命週期原則：
...
----
在日誌歸檔 S3 儲存桶上設定 S3 儲存桶生命週期原則時，對於符合儲存桶生命週期篩選器的物件，生命週期過期作業會置換 ILM 原則。不符合儲存桶生命週期篩選器的物件將使用 ILM 原則的保留設定。如果物件符合儲存桶生命週期篩選器，但未明確指定過期作業，則不會使用 ILM 原則的保留設定，並隱含表示物件版本將永久保留。
