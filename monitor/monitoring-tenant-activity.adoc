...
+
image:tenant_accounts_page.png["租戶帳戶頁面"]

. （可選）透過選擇 *Sign in/Copy URL* 欄中的登入連結 image:icon_tenant_sign_in.png["租戶登入圖示"] 登入租用戶帳戶。
. （可選）在 *Sign in/Copy URL* 欄中選取複製 URL 連結 image:icon_tenant_copy_url.png["租戶複製URL圖示"]，以複製租用戶登入頁面的 URL。
...
+
image:tenant_accounts_example_csv.png["租戶帳戶範例為.csv"]
...
+
image:tenant_usage_modal.png["租戶使用模式"]
...
+
image:tenant_bucket_space_consumption_GM.png["租戶庫位使用量長條圖"]
...
+
image:tenant_bucket_details_csv.png["租戶桶詳細資料CSV範例"]
