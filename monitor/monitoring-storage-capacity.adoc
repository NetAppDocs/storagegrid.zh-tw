...
+
image:dashboard_data_and_metadata_space_usage_breakdown.png["資料與中繼資料空間使用率明細"]

.. 記下儲存設備隨時間卡上的圖表。使用「時間週期」下拉式清單、協助您判斷儲存設備的使用速度。
+
image:dashboard_storage_over_time.png["隨時間推移的儲存"]
...
.. 選擇「*網格_*>*儲存設備*」。
+
image:nodes_deployment_storage_tab.png["節點部署儲存索引標籤"]
...
image:calculating_watermarks.gif["總可用空間的概念性繪圖"]
...
+
image:nodes_page_storage_used_object_data.png["節點頁面儲存使用的物件資料"]
...
NOTE: 若要查看這些值的圖表，請按一下「可用」欄中的圖表圖示 image:icon_chart_new_for_11_5.png["圖表圖示"]。

+
image:nodes_page_storage_tables.png["物件儲存資料表"]
...
+
image:storage_used_object_metadata.png["使用的儲存設備-物件中繼資料"]
...
image:forecast-metadata-usage.png["依站台預測中繼資料使用量"]
