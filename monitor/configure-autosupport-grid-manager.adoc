...
summary: 此 AutoSupport 功能可讓您的 StorageGRID 系統定期和根據事件向技術支援傳送運作健全狀況和狀態訊息，從而實現主動式監控和疑難排解。StorageGRID AutoSupport 也支援使用 Digital Advisor 進行預測性建議。 
---
...
[role="lead"]
此 AutoSupport 功能可讓您的 StorageGRID 系統定期和根據事件向技術支援傳送運作健全狀況和狀態訊息，從而實現主動式監控和疑難排解。StorageGRID AutoSupport 也支援使用 Digital Advisor 進行預測性建議。

預設情況下，當您首次安裝 StorageGRID 時，基於軟體的 StorageGRID 系統會啟用每週和事件觸發的 AutoSupport 功能。但是，您必須在每個應用裝置上<<autosupport-for-appliances,設定硬體 AutoSupport>>。如有需要，您可以變更 AutoSupport 組態。


NOTE: 如果要變更 StorageGRID AutoSupport 的組態，請僅在主要管理節點上進行變更。

.開始之前
* 您已使用 link:../admin/web-browser-requirements.html["支援的網頁瀏覽器"] 登入主要管理節點上的 Grid Manager。
* 您有link:../admin/admin-group-permissions.html["root 存取權限"]。
* 如果您使用 HTTPS 傳送 AutoSupport 套件，則您已向主要管理節點提供傳出網際網路存取，可以直接提供，也可以透過其他方式 link:../admin/configuring-admin-proxy-settings.html["使用 Proxy 伺服器"] 提供（不需要傳入連線）。
* 如果在 StorageGRID AutoSupport 頁面上選擇了 HTTP，則必須link:../admin/configuring-admin-proxy-settings.html["已設定 Proxy 伺服器"]以 HTTPS 方式轉送 AutoSupport 套件。NetApp 的 AutoSupport 伺服器將拒絕使用 HTTP 傳送的套件。
* 如果您使用 SMTP 作為 AutoSupport 套件的傳輸協定，您將擁有link:../monitor/configure-email-server.html["已設定電子郵件伺服器"]。
...
== [[configure-autosupport]]設定 AutoSupport

若要變更 AutoSupport 的安裝預設組態，或在每個 StorageGRID 應用裝置上設定硬體 AutoSupport：
...
** *HTTP*：如果您選擇 HTTP，則必須link:../admin/configuring-admin-proxy-settings.html["設定 Proxy 伺服器"]將 AutoSupport 封包轉送為 HTTPS。NetApp 的 AutoSupport 伺服器會拒絕使用 HTTP 傳送的封包。此協定使用 80 連接埠。
** *SMTP*：如果您希望透過電子郵件傳送 AutoSupport 套裝組合，請使用此選項。您必須先link:../monitor/configure-email-server.html["設定電子郵件伺服器"]。


+
您設定的協定用於傳送所有 AutoSupport 套件，但 AutoSupport on Demand 除外，該功能需要使用 HTTPS 協定。
...
. 如果您選擇了 *SMTP*，請使用 *Sender* 欄位輸入您先前設定的電子郵件伺服器位址。
...
若要驗證或變更用於傳送 AutoSupport 套件的通訊協定，請參閱以下說明 <<configure-autosupport,設定 AutoSupport>>。
...
如果您在 link:sending-eseries-autosupport-messages-through-storagegrid.html["系統管理程式SANtricity"] 中設定透過 Proxy 傳遞 AutoSupport，即可將來自 E 系列應用裝置的 AutoSupport 套件包含在 StorageGRID AutoSupport 中。
...
.相關資訊
* link:../network/external-communications.html["用於外部通訊的連接埠"]
