...
summary: 新增 Storage Nodes 後，您可以使用銷除編碼重新平衡程序，在現有和新的 Storage Nodes 之間重新分配銷除編碼片段。 
---
...
+
當所有節點中的可用空間與目標可用空間分割區之間的偏差在 5% 以內時，EC 重新平衡作業即被視為完成並停止。

** 如果為 `Success`、則 `State`可選擇性地<<review_object_storage,檢閱物件儲存>>查看網站的更新詳細資料。
+
站台上各個儲存節點的可用空間現在應該更加均衡。
