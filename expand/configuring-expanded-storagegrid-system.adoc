...
. 如果您新增了管理節點：
+
.. 如果您的 StorageGRID 系統已啟用單一登入 （SSO），請為新的管理節點建立信賴方信任。在建立此信賴方信任之前，您無法登入該節點。請參閱使用 link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] 或 link:../admin/learn-about-setting-up-sso-saml.html["SAML"] 設定 SSO。
