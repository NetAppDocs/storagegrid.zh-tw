...
summary: 您可以在管理節點和閘道節點上建立虛擬區域網路 （VLAN） 介面，並在 HA 群組和負載平衡器端點中使用這些介面來隔離和分割流量，以提升安全性、靈活性和效能。 
---
...
+
image:vlan-create-parent-interfaces.png["VLAN建立精靈父介面"]
...
** 如果需要刪除父級介面，請選擇垃圾桶圖示image:icon-trash-can.png["垃圾桶圖示"]。
