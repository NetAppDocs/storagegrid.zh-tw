...
* 連結成本由管理節點和閘道節點上的負載平衡器服務用於引導用戶端連線。請參閱link:../admin/managing-load-balancing.html["負載平衡考量"]。


此圖顯示一個由三個站點組成的網格，其中站點之間已設定連結成本：

image:link_costs.png["資料中心之間連結成本的概念性圖表"]
...
.步驟
. 選擇*支援* > *其他* > *連結成本*。
. 對於「站台/連結成本」欄中列出的每個站台，請為其他站台輸入 0 到 100 的成本值。
+
如果來源與目的地相同、則無法變更連結成本。

. 放棄或儲存變更：
+
** 若要捨棄變更，請選取 *Clear changes*。
** 若要儲存變更，請選取 *Save*。
