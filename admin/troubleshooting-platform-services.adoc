...
summary: 平台服務中使用的端點由租用戶使用者在 Tenant Manager 中建立和維護。但是，如果租用戶在設定或使用平台服務時遇到問題，您可以嘗試使用 Grid Manager 來協助解決問題。 
---
...
image:tenant_dashboard_endpoint_error.png["儀表板上的端點錯誤訊息"]

租用戶使用者可以前往「端點」頁面，查看每個端點的最新錯誤訊息，並確認錯誤發生的時間。「上次錯誤」欄位顯示每個端點的最新錯誤訊息，並指示錯誤發生的時間。包含 image:icon_alert_red_critical.png["紅色X圖示"] 圖示的錯誤表示過去 7 天內發生的錯誤。

image:endpoints_last_error.png["顯示「上次錯誤」欄的「端點」頁面的快照"]
...
. 檢視「要求錯誤率」圖表。
+
image:nodes_page_site_level_platform_services.gif["節點頁面站台層級平台服務"]
