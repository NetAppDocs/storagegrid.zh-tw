...
+
image:securitysettings_tls_ssh_policies_current.png["TLS 和 SSH 原則"]
...
.FIPS 嚴格
--
如果您需要通用準則（Common Criteria）認證，且需要使用 NetApp Cryptographic Security Module (NCSM) 3.1.2 或 NetApp StorageGRID Kernel Crypto API 6.12.63-1+ntap1 模組來進行與負載平衡器端點、租戶管理員和網格管理員的外部用戶端連線，請使用 FIPS 嚴格原則。使用此原則可能會降低效能。

NCSM 3.1.2 和 NetApp StorageGRID Kernel Crypto API 6.12.63-1+ntap1 模組用於下列操作：
...
+
如需更多資訊，請參閱 NIST Cryptographic Algorithm Validation Program link:https://csrc.nist.gov/projects/cryptographic-algorithm-validation-program/details?validation=40426["憑證 #A7816"^]。
...
+
如需更多資訊，請參閱 NIST Cryptographic Algorithm Validation Program link:https://csrc.nist.gov/projects/cryptographic-module-validation-program/entropy-validations/certificate/223["熵證書#E223"^]。
...
如果您的 StorageGRID 具有 FIPS 140 密碼編譯需求，則可選擇啟用 FIPS 模式功能，以使用 NCSM 3.1.2 和 NetApp StorageGRID Kernel Crypto API 6.12.63-1+ntap1 模組：
...
. 選取 * 複製到剪貼簿 * 、然後選取 * 取消 * 。
+
image:securitysettings_custom_security_policy_copy.png["複製現有原則以建立自訂原則"]
