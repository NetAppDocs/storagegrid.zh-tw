...
---
= 在 StorageGRID 中為 SAML 建立 PingFederate 服務供應商連接
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
使用 PingFederate 為系統中的每個管理節點建立 SAML 服務供應商 （SP） 連線。為了加快流程，您需要從 StorageGRID 匯入 SAML 中繼資料。

.開始之前
* 您已設定StorageGRID 單一登入以供使用、並選擇* Ping federate*作為SSO類型。
* 您在 Grid Manager 中擁有 link:../admin/configure-sso-saml.html#enter-sandbox-mode["進入沙盒模式"]。
* 您系統中每個管理節點都有* SP連線 ID *。您可以在「設定 SSO」頁面上的管理節點詳細資料表中找到這些值。
...
. 選擇<<password-validator,密碼認證驗證工具>>你創造的。
...
.步驟
. 在匯入中繼資料索引標籤上、選取*檔案*。
. 選擇從管理節點的設定 SSO 頁面下載的 SAML 元資料檔。
...
. 從SP連線索引標籤、選取*認證*。
. 從「認證」標籤中、選取*「設定認證」*。
. 選擇<<signing-certificate,簽署憑證>>您建立或匯入的。
. 選擇*下一步*以前往*管理簽名驗證設定*。
+
.. 在信任模式索引標籤上、選取*未鎖定*。
