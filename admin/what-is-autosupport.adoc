...
使用 AutoSupport 有助於更快解決問題。技術支援可監控系統的儲存需求，並協助判斷您是否需要新增節點或站台。AutoSupport 也可以將套件傳送至其他目的地。
...
NOTE: 如果未啟用 StorageGRID AutoSupport ，則會在 Grid Manager 儀表板上顯示訊息。此訊息包含AutoSupport 指向「資訊功能」組態頁面的連結。如果您關閉訊息，即使 AutoSupport 保持停用狀態，只有在瀏覽器快取清除之後，訊息才會再次出現。
