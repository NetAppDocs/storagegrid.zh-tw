...
summary: 如果要同步租戶詳細資料和複寫物件資料，可以在 StorageGRID 系統之間建立網格聯合連接。 
---
...
[role="lead"]
如果要同步租戶詳細資料和複寫物件資料，可以在 StorageGRID 系統之間建立網格聯合連接。

如圖所示、建立網格同盟連線包括兩個網格上的步驟。您可以在一個網格上新增連線、然後在另一個網格上完成連線。您可以從任一網格開始。


TIP: 您也可以選擇link:../admin/grid-federation-automate-connections.html["自動化建立、編輯及維護網格聯盟連線"]。

image:grid_federation_create_connection.png["在兩個網格上建立連線步驟的圖形總覽"]
...
. 選擇*配置* > *系統* > *網格聯合*。
. 選取 * 新增連線 * 。
. 輸入連線名稱，該名稱必須是唯一的，以便您識別此連線。例如， `Grid 1-Grid 2`。
. 如果需要link:../admin/configure-global-namespace.html["設定全域命名空間"]，請選擇 *Enable global namespace*。
. 選擇*繼續*。
. 輸入網格詳細資訊，如下所示：
...
* link:grid-federation-troubleshoot.html["疑難排解連線問題"]，包括解決與帳戶同步和跨網格複寫相關的任何警報和錯誤。
