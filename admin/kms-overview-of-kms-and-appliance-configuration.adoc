...
summary: '在使用金鑰管理伺服器 (KMS) 保護裝置節點上的 StorageGRID 資料之前，您必須設定一個或多個 KMS 伺服器，並為裝置節點啟用節點加密。完成這兩項設定任務後，金鑰管理程序將自動進行。' 
---
...
image:kms_configuration_overview.png["KMS 組態工作流程、如下列文字所述"]
