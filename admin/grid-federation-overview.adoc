...
summary: 您可以使用網格聯合來同步租戶，並在多個 StorageGRID 系統之間複寫其物件，以實現災難恢復。 
---
= 了解 StorageGRID 網格聯邦
:allow-uri-read: 
:icons: font
...
[role="lead"]
您可以使用網格聯合來同步租戶，並在多個 StorageGRID 系統之間複寫其物件，以實現災難恢復。
...
image:grid_federation_workflow.png["網格同盟連線工作流程"]
...
+
有關版本要求的詳細資訊，請參閱 https://library.netapp.com/ecm/ecm_download_file/ECMLP3373065["版本資訊"^]。您必須登入現有 NetApp 帳戶或建立帳戶才能存取發行說明。
...
* 必須至少存在一個網格同盟連線，才能使用 link:grid-federation-what-is-account-sync.html["帳戶同步"] 或 link:grid-federation-what-is-cross-grid-replication.html["跨網格複寫"]。
...
image:grid_federation_certificates.png["網格同盟憑證"]
