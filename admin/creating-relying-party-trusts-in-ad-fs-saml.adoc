---
permalink: admin/creating-relying-party-trusts-in-ad-fs-saml.html 
sidebar: sidebar 
...
---
= 在 AD FS 中為 StorageGRID 中的 SAML 建立信賴方信任
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
您必須使用 Active Directory Federation Services （AD FS） 為系統中的每個 SAML 管理節點建立信賴方信任。您可以透過 PowerShell 指令建立信賴方信任，也可以從 StorageGRID 匯入 SAML 中繼資料，或手動輸入資料。

.開始之前
* 您已設定StorageGRID 單一登入以供使用、並選擇* AD FS*作為SSO類型。
* 您在 Grid Manager 中擁有 link:../admin/configure-sso-saml.html["進入沙盒模式"]。
...
. 完成後，返回 StorageGRID 並link:../admin/configure-sso-saml.html#test-sso["測試所有依賴方信任"]確認其組態是否正確。
...
. 重複這些步驟、為StorageGRID 您的整套系統中的所有管理節點設定依賴方信任。
. 完成後，返回 StorageGRID 並link:../admin/configure-sso-saml.html#test-sso["測試所有依賴方信任"]確認其是否已正確設定。
