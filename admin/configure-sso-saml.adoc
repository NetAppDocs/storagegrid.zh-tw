...
summary: 您可以依照 SAML 單一登入 （SSO） 設定精靈的步驟操作，並進入沙盒模式，在為所有 StorageGRID 使用者啟用 SSO 之前，先設定和測試 SSO。啟用 SSO 後，您可以視需要返回沙盒模式來變更或重新測試組態。 
---
= 使用 SAML 在 StorageGRID 中設定 SSO
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
依照 SAML 單一登入 （SSO） 設定精靈的步驟操作，進入沙盒模式，在為所有 StorageGRID 使用者啟用 SSO 之前，先設定並測試單一登入 （SSO）。啟用 SSO 後，您可以根據需要返回沙盒模式來變更或重新測試組態。
...
為了允許 StorageGRID （服務供應商）和 SSO 身分供應商就使用者身分驗證請求進行安全通訊，您需要完成以下任務：

. 在StorageGRID中配置設定。
. 使用 SSO 身分提供者的軟體，為每個管理節點建立信賴方信任（AD FS）、企業應用程式（Entra ID）或服務供應商（PingFederate）。
...
== [[enter-sandbox-mode]]步驟 2：提供信賴方信任、企業應用程式或 SP 連線

根據您選擇的 SSO 類型，填寫精靈步驟 2 中的欄位。
...
. 依照link:../admin/creating-enterprise-application-entra-id-saml.html["在 Entra ID 中建立企業應用程式"]中的步驟，為表格中列出的每個管理節點建立一個企業應用程式。
...
+
有關說明，請前往 link:../admin/creating-relying-party-trusts-in-ad-fs-saml.html["在AD FS中建立依賴方信任"]。
...
. 前往 Azure 入口網站。
. 請依照 link:../admin/creating-enterprise-application-entra-id-saml.html["在 Entra ID 中建立企業應用程式"] 中的步驟，將每個管理節點的 SAML 中繼資料檔案上傳至其對應的 Entra ID 企業應用程式。
...
. 前往PingFedate。
. link:../admin/creating-sp-connection-ping-saml.html["建立一個或多個StorageGRID 服務供應商（SP）連線以供使用"]。使用每個管理節點的 SP 連線 ID（顯示在「設定 SSO」頁面上的表格中）以及您為該管理節點下載的 SAML 中繼資料。
+
...
Active Directory
 a| 
link:creating-relying-party-trusts-in-ad-fs-saml.html["為每個管理節點建立一個應用程式群組"]
...
進入 ID
 a| 
link:creating-enterprise-application-entra-id-saml.html["為每個管理節點建立一個應用程式"]
...
PingFedate
 a| 
link:creating-sp-connection-ping-saml.html["為每個管理節點建立服務供應商連線"]
