...
image:metadata_space_across_sites.png["跨站台的中繼資料空間"]
...
對於每個儲存節點具有 128 GB 或更多 RAM 的新 StorageGRID 12.1 或更高版本的安裝，預設設定為 20,000 GB （20 TB）。
...
+
image:storage_used_object_metadata_actual_reserved.png["使用的儲存設備-物件中繼資料-實際保留"]



快照中，「實際保留空間」值為 20 TB。此快照適用於新安裝的 StorageGRID 12.1 中的大型儲存節點。Volume 0 至少為 20 TB，因此使用預設的系統層級中繼資料保留空間設定。
...
假設您安裝了一個新的 StorageGRID 系統，版本為 12.1 或更高版本。在本例中，假設每個儲存節點擁有超過 128 GB 的 RAM，且儲存節點 1 （SN1） 的 Volume 0 為 6 TB。基於這些值：

* 系統級 *中繼資料保留空間* 設定為 20 TB。（如果每個儲存節點的 RAM 超過 128 GB，則這是新安裝 StorageGRID 12.1 或更高版本時的預設值。）
* SN1 的中繼資料實際保留空間為 6 TB。（由於 Volume 0 小於「中繼資料保留空間」設定值，因此整個 Volume 都已保留。）
...
image:metadata_allowed_space_volume_0.png["中繼資料允許空間Volume 0"]
...
&lt ； = 4 TB
 a| 
實際保留空間的 75% 用於中繼資料，最大可達 1.32 TB
 a| 
實際保留空間的 75% 用於中繼資料，最大可達 1.98 TB
...
> 4 TB
 a| 
（實際中繼資料保留空間 − 1 TB）× 75%，最高可達 1.32 TB
 a| 
（實際中繼資料保留空間 − 1 TB）× 75%，最大可達 7.92 TB
...
+
image:storage_used_object_metadata_allowed.png["使用的儲存設備-物件中繼資料-允許"]



在快照中，*允許* 值為 7.92 TB，這是儲存節點的最大值，該儲存節點的實際中繼資料保留空間超過 4 TB。
...
* 系統級 *中繼資料保留空間* 設定為 20 TB。（如果每個儲存節點的 RAM 超過 128 GB，則這是新安裝 StorageGRID 12.1 或更高版本時的預設值。）
* SN1 的中繼資料實際保留空間為 6 TB。（由於 Volume 0 小於「中繼資料保留空間」設定值，因此整個 Volume 都已保留。）
* 根據 <<table-allowed-space-for-metadata,允許用於中繼資料空間的表格>> 中所示的計算，SN1 上中繼資料的*允許*空間為 3.75 TB：（中繼資料的實際保留空間 − 1 TB）× 75%，最大可達 7.92 TB。
...
image:metadata_space_three_storage_nodes.png["中繼資料空間三個儲存節點"]
