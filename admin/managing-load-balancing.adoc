...
NOTE: 儘管StorageGRID負載平衡器服務是建議的負載平衡機制，但您可能希望整合第三方負載平衡器。如需了解更多信息，請聯繫您的NetApp客戶代表或參考以下內容： https://docs.netapp.com/us-en/storagegrid-enable/technical-reports/load-balancer-index.html["將第三方負載平衡器與StorageGRID結合使用"^]。
...
如果您打算使用 S3 虛擬主機風格的要求，則憑證也必須包含您已設定的每個 link:../admin/configuring-s3-api-endpoint-domain-names.html["S3 端點網域名稱"] 的*備用名稱*項目，包括任何萬用字元名稱。例如：
