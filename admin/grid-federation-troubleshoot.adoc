...
summary: 您可能需要排查與網格聯合連線、帳戶同步和跨網格複寫相關的警報和錯誤。 
---
...
[role="lead"]
您可能需要排查與網格聯合連線、帳戶同步和跨網格複寫相關的警報和錯誤。
...
.問題
您無法登入已同步的租戶帳戶。Tenant Manager 登入頁面上的錯誤訊息為「此帳戶的憑證無效。請重試。」

.詳細資料
您為租戶的本機 root 使用者和其他本機使用者設定的密碼會自動同步。但是，您可能需要在目的地網格上變更租戶的本機 root 使用者密碼，才能登入該網格上的租戶帳戶。
...
.問題
使用 *Use grid federation connection* 權限建立新租戶後，您會看到訊息「Tenant created without a sync」。
...
. 找到同步失敗的租戶帳戶。
. 選取租戶名稱以顯示詳細資料頁面。
. 選取 *Retry sync*。如果錯誤已解決，租戶帳戶現在將同步到另一個網格。
...
image:grid_federation_last_error.png["顯示網格同盟連線最後一個錯誤欄中訊息的螢幕擷取畫面"]
...
. 執行所有建議的操作。
+
例如，如果跨網格複寫的目的地儲存桶的版本控制功能被暫停，請重新啟用該儲存桶的版本控制功能。或者，如果跨網格複寫的目的地儲存桶的 S3 Object Lock 已停用，請啟用該儲存桶的 S3 Object Lock。
