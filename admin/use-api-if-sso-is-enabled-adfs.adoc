...
[role="lead"]
如果你有link:../admin/how-sso-works.html["已設定並啟用單一登入（SSO）"]並且您使用 Active Directory 作為 SSO 提供程序，則必須發出一系列 API 請求以取得對網格管理 API 或租用戶管理 API 有效的身份驗證令牌。
...
* 這 `storagegrid-ssoauth.py`Python 腳本，位於StorageGRID安裝檔目錄中(`./rpms`對於 RHEL， `./debs`適用於 Ubuntu 或 Debian，以及 `./vsphere`對於 VMware)。
* Curl要求的工作流程範例。
+
...
+
image:sso_auth_python_script_adfs.png["AD FS的SSO驗證指令碼"]
