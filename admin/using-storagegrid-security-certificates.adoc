...
*** link:creating-relying-party-trusts-in-ad-fs-saml.html#create-a-relying-party-trust-manually["手動指定依賴方信任的 SSO 憑證"]
...
伺服器
 a| 
用於驗證從 StorageGRID 雲端儲存資源池到外部儲存設備位置（例如 S3 Glacier 或 Microsoft Azure Blob 儲存體）的連線。每種雲端供應商類型都需要不同的憑證。
 a| 
* ILM *>*儲存資源池*
 a| 
