...
summary: 跨網格複寫是指在透過網格聯合連線連接的兩個 StorageGRID 系統中，自動複寫所選 S3 儲存桶中的物件。跨網格複寫需要帳戶同步。 
---
...
[role="lead"]
跨網格複寫是指在兩個以某種方式連接的 StorageGRID 系統中，在選定的 S3 儲存桶之間自動複寫物件link:grid-federation-overview.html["網格同盟連線"]。link:grid-federation-what-is-account-sync.html["帳戶同步"]跨網格複寫需要進行設定。
...
image:grid_federation_cgr_workflow.png["跨網格複寫工作流程"]
...
如果租戶帳戶擁有「使用網格聯合連線」權限，可使用一或多個 link:grid-federation-overview.html["網格同盟連線"]，則具有根存取權限的租戶使用者可以在每個網格的對應租戶帳戶中建立儲存貯體（「來源」儲存貯體和「目的地」儲存貯體）。這些儲存貯體：

* 可以有不同的名稱
* 可以有不同的區域
* 必須啟用版本設定
* 必須採用下列其中一種 S3 物件鎖定組態：
+
** 來源：*已停用*，目的地：*已停用*
** 來源：*已啟用*，目的地：*已啟用*
** 來源：*已停用*，目的地：*已啟用*
...
建立完兩個儲存桶後，您可以為其中一個或兩個儲存桶設定跨網格複寫。
...
image:grid_federation_cross_grid_replication_one_direction.png["顯示單向網格同盟連線的影像"]
...
image:grid_federation_cross_grid_replication.png["影像顯示複寫的單一方向、與雙向複寫比較"]
...
image:grid_federation_cross_grid_replication_delete.png["顯示在兩個網格上複寫用戶端刪除的影像"]
...
CAUTION: 您可以對現有儲存桶啟用 S3 物件鎖定，這可能會導致日後發生組態錯誤。


NOTE: 如果在來源儲存桶上啟用了 S3 Object Lock，請確保在目的地儲存桶上也啟用了此功能。如果在目的地儲存桶上停用了 S3 Object Lock，則在複寫期間會發生錯誤。請參閱 link:../admin/grid-federation-troubleshoot.html#cross-grid-replication-alerts-and-errors["跨網格複寫警示和錯誤"]。
...
[[putobjectretention-and-putobjectlegalhold-arent-supported]]
