...
sidebar: sidebar 
keywords:  
summary: 了解 StorageGRID 12.1.0 中已移除或棄用的功能和特性。升級前，請檢查已移除或棄用的功能是否會影響您的用戶端應用程式或網格組態。 
---
...
[role="lead"]
了解 StorageGRID 12.1.0 中已移除或棄用的功能和特性。升級前，請檢查已移除或棄用的功能是否會影響您的用戶端應用程式或網格組態。
...
已過時:: 您*不應該*在新生產環境中使用此功能。現有生產環境可以繼續使用此功能。
...
12.3
 a| 
從 StorageGRID 12.1.0 開始，對新儲存物件靜態資料進行 AES-128 加密的支援已過時。對於最初以 AES-128 加密的儲存物件，仍繼續支援其擷取作業。
...
. 停用所有歸檔節點。請參閱 https://docs.netapp.com/us-en/storagegrid-118/maintain/grid-node-decommissioning.html["Grid 節點汰換（ StorageGRID 11.8 doc 網站）"^]。
. 從儲存設備集區和 ILM 原則中移除所有歸檔節點參考。請參閱 https://kb.netapp.com/hybrid/StorageGRID/Maintenance/StorageGRID_11.9_software_upgrade_resolution_guide["NetApp 知識庫： StorageGRID 11.9 軟體升級解決方案指南"^]。
...
12.1
 a| 
https://docs.netapp.com/us-en/storagegrid-118/monitor/managing-alarms.html#configure-notifications-for-alarms-legacy-system["設定警報通知（舊系統）（StorageGRID 11.8）"^]
