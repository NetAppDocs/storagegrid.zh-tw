...
summary: StorageGRID 所使用的 S3 REST API 會定期更新。 
---
= StorageGRID S3 REST API 支援變更
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
StorageGRID 所使用的 S3 REST API 會定期更新。
...
* `If-Match`
+
The `If-Match`標頭的行為會根據您的 StorageGRID 組態而有所不同。提供此標頭可能會傳回錯誤訊息（501 Not Implemented）。如果 `If-Match`標頭被接受但無法運作，則可能會觸發警示，其中包含有關啟用更嚴格的 501 Not Implemented 錯誤行為的說明。

* `If-None-Match`
+
The `If-None-Match`標頭的行為會根據您的 StorageGRID 組態而有所不同。提供此標頭可能會傳回錯誤訊息（501 Not Implemented）。如果 `If-None-Match`標頭被接受但無法運作，則可能會觸發警示，其中包含有關啟用更嚴格的 501 Not Implemented 錯誤行為的說明。

* 為以下請求和支援的標頭新增了對預先計算 Checksum 值的支援：
+
** CompleteMultipartUpload： `x-amz-checksum-crc32`， `x-amz-checksum-crc32c`，和 `x-amz-checksum-crc64nvme`
** CreateMultipartUpload： `CRC32`， `CRC32C`，以及 `CRC64NVME`，適用於 `x-amz-checksum-algorithm`
** CopyObject： `CRC32`， `CRC32C`， `CRC64NVME`，以及 `SHA256`（適用於 `x-amz-checksum-algorithm`）
** PutObject： `x-amz-checksum-crc32`、 `x-amz-checksum-crc32c`和 `x-amz-checksum-crc64nvme`
** UploadPart： `x-amz-checksum-crc32`、 `x-amz-checksum-crc32c`和 `x-amz-checksum-crc64nvme`


* 新增了對  `x-amz-checksum-type` 的支援，適用於 CreateMultipartUpload 和 CompleteMultipartUpload。
* 增加了對 `s3:x-amz-copy-source`原則條件索引鍵的支援。
* 允許在現有儲存桶上啟用 S3 物件鎖定。
...
* 新增對 STS AssumeRole 和工作階段原則的支援。請參閱 link:../s3/example-session-policies.html["會話策略範例"]。您可以在租戶群組下設定 AssumeRole。
...
11.7
 a| 
* 已新增link:../s3/quick-reference-support-for-aws-apis.html["快速參考：支援的 S3 API 要求"]。
...
|===
請參閱 link:../s3/index.html#supported-versions["S3 REST API 支援的版本與更新"] 以取得更多資訊。
