...
sidebar: sidebar 
keywords: StorageGRID, hardening guidelines, nodes, security, bmc password 
...
[[block-remote-ipmicontrol-remote-ipmi-access-to-bmc]]
== [[block-remote-ipmi]]控制遠端 IPMI 對BMC的訪問

您可以為所有包含 BMC 的應用裝置啟用或停用遠端 IPMI 存取。遠端 IPMI 介面允許任何擁有 BMC 帳戶和密碼的使用者對您的 StorageGRID 應用裝置進行底層硬體存取。如果您不需要對 BMC 進行遠端 IPMI 存取，請停用此選項。

* 若要在 Grid Manager 中控制對 BMC 的遠端 IPMI 存取，請前往 *Configuration* > *Security* > *Security settings* > *Appliances* > *Remote IPMI access*：
+
...
[[change-appliance-bmc-password]]
== 更改應用裝置 BMC 密碼

您可以變更應用裝置的 BMC 密碼，以限制存取並提高系統安全性。

. 在 Grid Manager 中，前往 *節點* > *_grid node_* > *BMC 設定*。
. 在「變更密碼」部分，在「新密碼」中輸入長度為 8 到 16 個字元的新 BMC 密碼，然後在「確認新密碼」中再次輸入。
. 選擇*保存*。


您也可以使用  `PUT /bmc-configuration/root-password` API 變更 BMC 密碼。



[[control-remote-access-through-usb-portscontrol-external-usb-port-access-on-appliances]]
== [[control-remote-access-through-usb-ports]]控制應用裝置外部 USB 連接埠的存取

您可以啟用應用裝置上的 USB 連接埠，以便技術支援存取進行診斷和疑難排解。如果您不需要存取 USB 連接埠，請基於安全性考量停用這些連接埠。初始安裝或升級後，USB 連接埠預設為啟用。

* 在 Grid Manager 中，前往 *Configuration* > *Security* > *Security settings* > *Appliances* > *USB port access*，啟用或停用 USB 連接埠：
+
** 選取 *Enable USB ports* 核取方塊並儲存以啟用連接埠。
+
如果之前未選取 USB 複選框並已儲存，請勾選 *Enable USB ports* 複選框並儲存以再次啟用 USB 連接埠。

** 取消選取「啟用 USB 連接埠」核取方塊並儲存以停用這些連接埠。
...
* 不要允許不受信任的用戶端直接連線至儲存節點。請使用由 Gateway Node 或第三方負載平衡器提供的負載平衡器端點。
