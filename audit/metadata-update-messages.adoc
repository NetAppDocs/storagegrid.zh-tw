...
S3 用戶端發出請求 (SUPD) 以更新 S3 物件 (S3KY) 的指定中繼資料 (`x-amz-meta-*`。

在本例中，請求標頭包含在 HTRH 欄位中，因為它已設定為稽核協定標頭（*組態* > *監控* > *稽核和 syslog 伺服器*）。請參閱link:../monitor/configure-log-management.html["設定日誌管理和外部系統日誌伺服器"]。
