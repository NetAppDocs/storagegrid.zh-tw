...
summary: 您可以使用「變更 IP」工具變更一個或多個節點的網路組態。您可以變更網格網路的組態，也可以新增、變更或移除管理網路或用戶端網路。您也可以變更應用裝置節點 BMC 的 LAN IP 設定。 
---
= 變更 StorageGRID 中的節點網路組態
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
您可以使用「變更 IP」工具變更一個或多個節點的網路組態。您可以變更網格網路的組態，也可以新增、變更或移除管理網路或用戶端網路。您也可以變更應用裝置節點 BMC 的 LAN IP 設定。
...
NOTE: 如果您的 StorageGRID 系統已啟用單一登入 (SSO)，且您正在變更管理節點的 IP 位址，請注意，任何使用管理節點的 IP 位址（而非建議的完整網域名稱）設定的信賴方信任都將失效。您將無法再登入該節點。變更 IP 位址後，您必須立即使用新的 IP 位址在 Active Directory Federation Services (AD FS) 中更新或重新設定節點的信賴方信任。請參閱使用 link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] 或 link:../admin/learn-about-setting-up-sso-saml.html["SAML"] 設定 SSO 的說明。
...
+
image:change_ip_tool_main_menu.png["顯示變更IP工具歡迎畫面的快照"]
...
+
image:change_ip_tool_edit_ip_mask_sample_output.png["周邊文字所述的螢幕擷取畫面"]
...
+
image:change_ip_tool_validate_sample_error_messages.gif["周邊文字所述的螢幕擷取畫面"]

+
在此範例中、驗證已通過。

+
image:change_ip_tool_validate_sample_passed_messages.gif["周邊文字所述的螢幕擷取畫面"]
...
== 更改應用裝置節點 BMC LAN IP 設定

您可以變更應用裝置節點 BMC 的 LAN IP 設定。

. 在 Grid Manager 中，前往 *節點* > *_grid node_* > *BMC 設定*。
. 在 LAN IP 設定部分，選擇 *靜態* 或 *DHCP* 的 IP 指派類型。
. 輸入 BMC 的 MAC 位址、IPv4 位址（CIDR）和預設閘道。
. 選擇*保存*。


您也可以使用 `PUT /bmc-configuration/root-password`或 `GET /bmc-configuration/root-password` API 來變更或檢視 BMC LAN IP 設定。
