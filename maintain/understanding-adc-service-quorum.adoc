...
summary: 如果停用後仍有太少的管理網域控制站（ ADC ）服務、您可能無法取消委任某個站台上的特定儲存節點。 
---
...
如果需要停用某個儲存節點，可以透過link:../release-notes/changes-to-grid-management-api.html#new-private-endpoints-for-move-adc["將 ADC 服務移至同一站點上的另一個儲存節點"]或在link:../expand/index.html["擴充"]中新增一個新的儲存節點並指定其應具有 ADC 服務來繼續滿足 ADC 仲裁要求。然後，您可以停用現有的儲存節點而不會影響仲裁。
