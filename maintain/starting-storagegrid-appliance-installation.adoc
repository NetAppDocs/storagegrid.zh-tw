...
* 您需要指定主要管理節點的 IP 位址。如果主要管理節點無法使用，則需要指定非主要管理節點的 IP 位址（如果可用）。您還需要確認節點的主機名稱（系統名稱）。
...
. 在「節點名稱」欄位中，輸入與要恢復的節點相同的主機名稱（系統名稱），然後選擇「儲存」。
. 在「安裝」部分，確認目前狀態為「準備開始將  `_node name_` 安裝到主要管理節點 `_admin_ip_` 的網格中」，且「開始安裝」按鈕已啟用。
+
...
NOTE: 如果需要手動存取 Monitor 安裝頁面，請從功能表列中選取 *Monitor installation*。參見 https://docs.netapp.com/us-en/storagegrid-appliances/installconfig/monitoring-appliance-installation.html["監控應用裝置安裝"^]。
