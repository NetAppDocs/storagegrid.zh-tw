...
* 系統必須始終包含足夠的儲存節點以滿足運作需求，包括 link:understanding-adc-service-quorum.html["ADC 仲裁"] 和使用中的 link:reviewing-ilm-policy-and-storage-configuration.html["ILM原則"]。如果退役作業會導致站台的 ADC 數量低於法定人數，StorageGRID 會自動將 ADC 服務移至同一站台上可用的非 ADC 儲存節點，以維持 ADC 法定人數。如果 ADC 服務無法自動移動，您可能需要先 link:../expand/adding-grid-nodes-to-existing-site-or-adding-new-site.html["在擴充作業中新增儲存節點"]，才能退役現有的儲存節點。
+
