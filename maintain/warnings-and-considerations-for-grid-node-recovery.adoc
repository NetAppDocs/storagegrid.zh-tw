...
* 如果您要恢復的網格節點不是主要管理節點，則正在恢復的網格節點與管理恢復程序的管理節點之間必須存在連線。
...
如果託管多個網格節點的伺服器發生故障，您可以按任意順序恢復節點。但是，如果發生故障的伺服器託管著主要管理節點，且不存在非主要管理節點，則必須先恢復主要管理節點。
