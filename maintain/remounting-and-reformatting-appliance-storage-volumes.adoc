...
summary: 如果自動程式未能重新掛載已保留的儲存設備磁碟區並重新格式化故障的儲存設備磁碟區，則必須手動執行兩個指令碼來完成這些作業。第一個指令碼重新掛載已正確格式化為 StorageGRID 儲存設備磁碟區的磁碟區。第二個指令碼重新格式化所有未掛載的磁碟區，並在需要時重建 Cassandra 資料庫，然後啟動服務。 
---
= 重新掛載並重新格式化 StorageGRID 應用裝置儲存磁碟區（手動步驟）
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
如果自動程式未能重新掛載已保留的儲存設備磁碟區並重新格式化故障的儲存設備磁碟區，則必須手動執行兩個指令碼來完成這些作業。第一個指令碼重新掛載已正確格式化為 StorageGRID 儲存設備磁碟區的磁碟區。第二個指令碼重新格式化所有未掛載的磁碟區，並在需要時重建 Cassandra 資料庫，然後啟動服務。

在 Storage Node 恢復期間，如果 Grid Manager 中出現「等待手動步驟」訊息，請依照下列步驟重新掛載和重新格式化儲存設備磁碟區。如果未看到此訊息，則重新掛載和重新格式化操作已自動完成，您可以跳過_此_程序link:../maintain/restoring-object-data-to-storage-volume-for-appliance.html["還原物件資料"]。
...
+
image:recovering_cassandra.png["顯示Grid Management Interface恢復進度的快照"]

. 指令碼在節點上啟動服務之後 `sn-recovery-postinstall.sh`、您可以將物件資料還原至指令碼格式化的任何儲存磁碟區。
+
指令碼會詢問您是否要使用 Grid Manager Volume 還原程序來還原物件資料。

+
** 大多數情況下，您應該link:../maintain/restoring-object-data-to-storage-volume-for-appliance.html["使用 Grid Manager 還原物件資料"]。回答 `y`以使用 Grid Manager。
** 在極少數情況下，例如當您知道替換節點可用於物件儲存的 Volume 比原始節點少時，請聯絡技術支援以手動還原物件資料。如果其中一種情況適用，請回答 `n`並按任意鍵返回命令列。
