...
.關於這項工作
您可以透過在 *Health* 欄中尋找藍色的「未知」圖示 image:icon_alarm_blue_unknown.png["圖示未知"] 或灰色的「管理關閉」圖示 image:icon_alarm_gray_administratively_down.png["圖示管理性關閉"] 來識別斷開連線的節點。
...
. 新的恢復套件可用後，選擇連結或選擇 *Maintenance* > *System* > *Recovery package* 以存取恢復套件頁面。然後，下載  `.zip` 檔案。
+
...
. 如果您要退役儲存節點，請監控在退役過程中自動啟動的*複製資料*和*糾刪碼 (EC) 資料*修理作業的狀態。
+
[role="tabbed-block"]
====
.複寫資料
...
.銷毀編碼（ EC ）資料
--
若要監控糾刪碼資料的修理並重試任何可能失敗的請求，請選擇 *Support* > *Tools* > *Metrics*，檢視目前作業的預計完成時間和完成百分比。然後，在 Grafana 部分選擇 *EC Overview*。查看 *Grid EC Job Estimated Time to Completion* 和 *Grid EC Job Percentage Completed* 儀表板。
