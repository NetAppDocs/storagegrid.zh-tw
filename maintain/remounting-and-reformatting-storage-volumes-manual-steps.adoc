...
在儲存節點恢復期間，如果 Grid Manager 中出現「等待手動步驟」訊息，請依照下列步驟重新掛載和重新格式化儲存設備磁碟區。如果未看到此訊息，則重新掛載和重新格式化操作已自動完成，您可以跳過此程序並link:../maintain/restoring-object-data-to-storage-volume.html["還原物件資料"]。
...
+
** 大多數情況下，您應該link:../maintain/restoring-object-data-to-storage-volume.html["使用 Grid Manager 還原物件資料"]。回答 `y`以使用 Grid Manager。
