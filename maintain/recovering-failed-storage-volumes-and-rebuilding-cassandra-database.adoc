...
---
= 在 StorageGRID 中恢復故障的儲存磁碟區並重建 Cassandra 資料庫
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.接下來呢？
儲存設備磁碟區重新格式化並重新掛載，必要的 Cassandra 操作完成後，您可以link:../maintain/restoring-object-data-to-storage-volume-where-system-drive-is-intact.html["使用 Grid Manager 還原物件資料"]。
