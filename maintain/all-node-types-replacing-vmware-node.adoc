...
---
= 將 StorageGRID 中的 VMware 節點替換為恢復節點
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. 部署一個新的虛擬機器作為取代節點，並將其連接到一個或多個StorageGRID網路。有關說明，請參閱link:../swnodes/deploying-storagegrid-node-as-virtual-machine.html["將StorageGRID 節點部署為虛擬機器"]。
+
...
管理節點
 a| 
link:recovering-from-admin-node-failures.html["StorageGRID 中的主要或非主要管理節點恢復"]
