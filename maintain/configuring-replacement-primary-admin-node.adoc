...
. 選擇 *Recover a failed primary Admin Node*。
+
image:install_or_recover_primary_admin_node.png["安裝或恢復主管理節點"]

. 上傳恢復包的最新備份：
+
.. 選擇*瀏覽*。
.. 找到 StorageGRID 系統的最新恢復套件檔案，然後選擇 * 開啟 *。
...
NOTE: 若要設定信賴方信任，請參閱使用 link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] 或 link:../admin/learn-about-setting-up-sso-saml.html["SAML"] 設定 SSO。若要存取預設伺服器憑證，請登入管理節點的命令 shell。前往  `/var/local/mgmt-api` 目錄，然後選取  `server.crt` 檔案。
