...
CAUTION: 如果此儲存節點處於唯讀維護模式，以便其他儲存節點能夠從儲存磁碟區發生故障的儲存節點擷取物件，請先恢復儲存磁碟區發生故障的儲存節點上的磁碟區，再還原此故障儲存節點。請參閱相關說明 link:reviewing-warnings-about-storage-volume-recovery.html["從系統磁碟機完好無損的儲存磁碟區故障中恢復"]。
