...
NOTE: 為生產級 StorageGRID 安裝指定外部 NTP 來源時，請勿使用 Windows Server 2016 之前的 Windows 版本上的 Windows 時間 （W32Time） 服務。舊版 Windows 上的時間服務精確度不足，Microsoft 不支援在高精度環境（包括 StorageGRID）中使用該服務。如需詳細資訊，請參閱 https://learn.microsoft.com/en-us/troubleshoot/windows-server/active-directory/support-boundary-high-accuracy-time["支援高精度時間的邊界"^]。
