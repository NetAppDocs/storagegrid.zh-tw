...
如果實體或虛擬 Linux 主機上託管了多個網格節點，您可以以任意順序恢復這些網格節點。但是，如果主要管理節點存在，則必須先恢復它，以避免其他網格節點在嘗試聯絡主要管理節點時阻塞恢復。如果您無法先恢復主要管理節點，但存在非主要管理節點，則可以使用該非主要管理節點以任意順序恢復節點。
