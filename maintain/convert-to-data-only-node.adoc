...
---
= 將 StorageGRID Storage Node 轉換為僅資料節點
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* 儲存節點不是僅元資料節點或僅資料節點。
* 如果儲存節點包含 ADC 服務，則網格中有兩個以上的 ADC 儲存節點線上，並且同一站台上有非 ADC 儲存節點可用於主控 ADC 服務。
...
.關於這項工作
使用此程序可將一個或多個組合儲存節點轉換為僅資料節點。如果儲存節點包含 ADC 服務，則在轉換過程中，該服務將自動移至同一站台上的另一個儲存節點。
...
NOTE: StorageGRID 會將 ADC 服務移至符合託管該服務要求的最新儲存節點。
