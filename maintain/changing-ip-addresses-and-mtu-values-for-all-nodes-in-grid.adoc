...
NOTE: 如果您的 StorageGRID 系統已啟用 SSO，且任何信賴方信任都是使用管理節點 IP 位址（而非建議的完整網域名稱）設定的，請在變更 IP 位址後，立即在 Active Directory Federation Services （AD FS） 中更新或重新設定這些信賴方信任。請參閱使用 link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] 或 link:../admin/learn-about-setting-up-sso-saml.html["SAML"] 設定 SSO 的相關資訊。
