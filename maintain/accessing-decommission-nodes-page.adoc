...
** 輸入搜尋詞彙以快速尋找特定節點。
+
在此範例中、「可退役」欄中的綠色核取記號表示您可以退役 API Gateway 節點和僅含資料的儲存節點。其他節點因各種原因無法退役。例如、主要管理節點無法退役、因為不支援退役此類型的節點。僅含中繼資料的儲存節點無法退役、因為退役後站台將不再符合 link:../maintain/understanding-adc-service-quorum.html["ADC 仲裁"]。

+
image:decommission_nodes_page.png["取消委任節點頁面"]
...
** image:icon_alarm_gray_administratively_down.png["灰色問號圖示"]（灰色）：管理性關閉
** image:icon_alarm_blue_unknown.png["藍色問號圖示"]（藍色）：未知
...
編輯HA群組以移除節點的介面、或移除整個HA群組。請參閱。 link:../admin/configure-high-availability-group.html["設定高可用度群組"]
 ...
 a| 
不，網站 _x_ 至少需要 3 個具備 ADC 服務的儲存節點，或有可用的非 ADC 節點可供移轉 ADC 服務。如果任何節點離線，則無法退役 ADC 節點。
 a| 
*僅限儲存節點*（包括組合儲存節點或僅儲存元資料的儲存節點）。如果站台中剩餘節點不足以支援 ADC 仲裁要求，且 ADC 服務無法自動移動，則無法退役儲存節點。自動移動需要網格中有兩個以上的 ADC 儲存節點處於線上狀態，且同一站台上有可用的非 ADC 儲存節點。
 a| 
確保符合自動 ADC 服務移動的條件（請參閱 link:../maintain/understanding-adc-service-quorum.html["ADC 仲裁"]），或 link:../maintain/move-adc-service.html["手動移動 ADC 服務"] 至同一站台上的另一個儲存節點。您也可以透過向站台新增新的儲存節點並指定其應具有 ADC 服務來執行擴充。請參閱 link:../expand/adding-grid-nodes-to-existing-site-or-adding-new-site.html["將網格節點新增至 StorageGRID 的工作流程概述"]。
