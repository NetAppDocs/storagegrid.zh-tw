...
當您關閉網站或網格中的所有節點時，儲存節點離線期間您將無法存取已擷取的物件。
...
.關於這項工作
首先停止管理節點和閘道節點上的服務，然後停止儲存節點上的服務。
...
NOTE: 如果主機包含多個網格節點，則在主機通電時，這些節點會自動重新上線。

.步驟
. 開啟主要管理節點和任何非主要管理節點的主機。
+

NOTE: 在儲存節點重新啟動之前，您無法登入管理節點。
