...
---
= 配置 StorageGRID ILM 以進行 FabricPool 數據
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* 如果您是從早於 StorageGRID 11.7 的版本升級至 StorageGRID 12.1，則您已設定了要使用的儲存資源池。通常，您應該為每個用於儲存資料的 StorageGRID 站台建立一個儲存資源池。（從 11.7 版開始，系統會自動為每個站台建立儲存資源池。）
+
