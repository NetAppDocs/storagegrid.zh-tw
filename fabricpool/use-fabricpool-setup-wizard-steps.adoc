...
. 如果儀表板上出現「* FabricPool and S3 設定精靈*」橫幅，請選擇橫幅中的連結。如果橫幅不再顯示，請從 Grid Manager 的標題列中選擇說明圖示 image:icon_help_menu_bar.png["說明圖示"]，然後選擇「* FabricPool and S3 設定精靈*」。
...
.步驟
. （可選）選擇複製圖示（image:icon_tenant_copy_url.png["複製圖示"]）將存取金鑰 ID 和秘密存取金鑰複製到剪貼簿。
+
