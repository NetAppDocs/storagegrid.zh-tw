...
* 多部分上傳物件是否包含在 link:../tenant/what-is-branch-bucket.html["分支桶"] 中，取決於發起擷取的時間和分支儲存桶的時間篩選器：
+
** *之前*。如果擷取是在分支儲存桶的 _Before_ 時間戳記之前啟動，則無論上傳何時完成，都會包含多部分上傳的物件。
** *之後*。如果擷取是在分支儲存桶的 _After_ 時間戳記之後發起的，則會包含多部分上傳的物件。
** *介於*之間。如果擷取是在分支儲存桶的 _From_ 和 _To_ 時間戳記之間啟動，則無論上傳何時完成，都會包含多部分上傳的物件。
** *排除*。如果擷取是在分支儲存桶的 _Exclude from_ 時間之前或 _Exclude to_ 時間之後發起的，則多部分上傳的物件將包含在內。
