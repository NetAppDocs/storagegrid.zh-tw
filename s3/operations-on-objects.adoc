...
*  `If-Match` 和  `If-None-Match` 標頭的行為會因 StorageGRID 組態而有所不同。提供這些標頭可能會傳回錯誤訊息（501 Not Implemented）。如果  `If-Match` 或  `If-None-Match` 標頭被接受但無法運作，則可能會觸發警示，其中包含有關啟用更嚴格的 501 Not Implemented 錯誤行為的說明。
