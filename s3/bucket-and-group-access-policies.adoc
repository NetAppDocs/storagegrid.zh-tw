...
IfExl主義 者
 a| 
附加至任何條件運算子（ Null 條件除外），以檢查是否缺少該條件金鑰。如果條件鍵不存在則返回 true 。
...
S3：PuttObject
 a| 
與 PuttObject ， CopyObject 和 CreateMultpartUpload 要求中的要求標頭展開的比較 `object-lock-mode`。
...
S3：PuttObjectRetention
 a| 
比較 `object-lock-mode` PutObjectRetention 要求中從 XML 實體展開的內容。
...
當物件要求包含標記時、需要特定的標記金鑰和值。
 ...
 a| 
S3:x-amz-server-Side 加密 - 客戶演算法
 a| 
S3：PuttObject
 a| 
與 PuttObject ， CopyObject ， CreateMultpartUpload ， UploadPart ， UploadPartCopy 和 CompleteMultpartUpload 要求中的要求標頭中的或 `copy-source-sse-customer-algorithm`展開的比較 `sse-customer-algorithm`。
