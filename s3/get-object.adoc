...
* `Accept-Encoding`
+
當物件沒有儲存的 `Content-Encoding`時，帶有 `Accept-Encoding: gzip`的 GET 請求會對回應進行動態壓縮。只有 `gzip`會影響 GET 回應的編碼； `deflate`、 `br`和 `identity`則不會。對於已儲存 `Content-Encoding`的物件，StorageGRID 會傳回儲存的編碼，且不會根據 `Accept-Encoding`套用額外的動態壓縮。StorageGRID 不支援帶有 `Accept-Encoding: gzip`的 GET 請求的範圍讀取操作。
