...
summary: 'StorageGRID 使用 Amazon Web Services (AWS) 原則語言，讓 S3 租用戶控制對儲存桶及其內部物件的存取。' 
---
...
== [[use-putoverwriteobject-permission]]使用 PutOverwriteObject 權限
...
如果存在
 a| 
附加到除 Null 條件之外的任何條件運算子，以檢查該條件索引鍵是否不存在。如果條件索引鍵不存在，則傳回 TRUE。
...
IP營運者
 a| 
與傳送請求的 IP 位址進行比較。可用於儲存桶或物件操作。

*注意：*如果 S3 請求是透過管理節點和閘道節點上的負載平衡器服務發送的，則此請求將與負載平衡器服務的上游 IP 位址進行比較。

*注意*：如果使用第三方非透明負載平衡器，則會與該負載平衡器的 IP 位址進行比較。任何 `X-Forwarded-For`標頭都將被忽略，因為無法確定其有效性。
...
資源/身分識別
 a| 
與傳送請求的傳送者使用者名稱進行比較。可用於儲存桶或物件操作。
...
S3：listBucketVerions權限
 a| 
與 ListObjects 或 ListObjectVersions 請求中指定的分隔符號參數進行比較。
...
S3：listBucketVerions權限
 a| 
與 ListObjects 或 ListObjectVersions 請求中指定的 max-keys 參數進行比較。
...
S3：listBucketVerions權限
 a| 
與 ListObjects 或 ListObjectVersions 請求中指定的前綴參數，或為 GetObject 或 HeadObject 指定的金鑰進行比較。
...
S3：PuttObject
 a| 
與 PutObject 和 UploadPartCopy 請求標頭中展開的 x-amz-copy-source 進行比較。
...
鏟斗
 a| 
有效。物件歸建立者帳戶所有，儲存桶原則不適用。建立者帳戶必須使用物件 ACL 授予物件的存取權限。
 a| 
...
NOTE: 在 S3 原則中將 DeleteObject 設定為 DENY，並不能防止 ILM 在存在「30 天後零複本」之類的規則時刪除物件。
