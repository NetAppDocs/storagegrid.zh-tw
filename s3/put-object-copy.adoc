...
* 如果鍵名或值的解釋值包含不可列印字元，StorageGRID 不會傳回 `x-amz-missing-meta`標頭。
...
* `Content-Type`
* `x-amz-checksum-algorithm`：指定 `CRC32`、 `CRC32C`、 `CRC64NVME`或 `SHA256`
+
複製物件時，如果來源物件具有 `checksum`或包含 `x-amz-checksum-algorithm`，則複製的物件具有 FULL_OBJECT Checksum。

+
如果來源物件具有 Checksum，但請求中未包含 `x-amz-checksum-algorithm`或使用了不支援的 Checksum 值，則複製的物件將具有與來源物件相同類型的 Checksum。StorageGRID 會忽略無效或不支援的 `x-amz-checksum-algorithm`值。

+
如果來源物件沒有 Checksum，或不包含 `x-amz-checksum-algorithm`，或使用了不支援的 Checksum 值，則複製的物件也沒有 Checksum。

+

NOTE: 如果 CopyObject 請求未指定 Checksum，且來源物件沒有 Checksum，StorageGRID 不會為物件計算預設的 CRC-64/NVME Checksum。
