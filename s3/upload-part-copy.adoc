...
StorageGRID 會自動偵測 CreateMultipartUpload 請求中是否指定了 Checksum，並在 UploadPartCopy 操作中使用此 Checksum。您無需在 UploadPartCopy 請求中指定 Checksum。如果 CreateMultipartUpload 請求未指定 Checksum，StorageGRID 不會為物件計算預設的 CRC-64/NVME Checksum。當 CreateMultipartUpload 未指定 Checksum 時，StorageGRID 不會為複製的部分包含 Checksum。
