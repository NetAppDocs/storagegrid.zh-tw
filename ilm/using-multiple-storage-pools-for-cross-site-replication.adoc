...
image:ilm_replication_at_2_sites.png["在 2 個站台每個複本複本 1 個"]

image:ilm_replication_make_2_copies_2_pools_2_sites.png["在 2 個站台中的每個站台建立 1 個複本 - 站台專屬儲存池"]
...
image:ilm_erasure_coding_site_loss_protection_4+2.png["EC 1 儲存池 9 儲存節點 3 站台"]
...
image:ec_three_sites_4_plus_2_site_loss_example_template.png["使用 4+2 EC 方案在 3 個站台上進行銷毀編碼、以儲存物件"]

如果某個站台遺失、資料仍可恢復：

image:ec_three_sites_4_plus_2_site_loss_example.png["銷毀編碼 4+2 EC 方案 1 站台遺失"]
