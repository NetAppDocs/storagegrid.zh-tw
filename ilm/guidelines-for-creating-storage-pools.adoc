...
+
處理量的下降會影響物件擷取和檢索的最大可達速率（當選擇 Balanced 或 Strict 作為擷取行為時），或可能導致 ILM 佇列積壓（當選擇 Dual commit 作為擷取行為時）。請參閱link:what-ilm-rule-is.html#ilm-rule-ingest-behavior["ILM 規則擷取行為"]。
