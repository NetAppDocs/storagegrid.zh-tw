...
summary: 雲端儲存資源池指定單一外部 Amazon S3 儲存桶或其他 S3 相容供應商或 Azure Blob Storage Container。 
---
...
[role="lead"]
雲端儲存資源池指定單一外部 Amazon S3 儲存桶或其他 S3 相容供應商或 Azure Blob Storage Container。

建立雲端儲存資源池時，您可以指定 StorageGRID 將用於儲存物件的外部儲存桶或 Container 的名稱和位置、雲端供應商類型（Amazon S3/GCP 或 Azure Blob Storage）以及 StorageGRID 存取外部儲存桶或 Container 所需的資訊。
