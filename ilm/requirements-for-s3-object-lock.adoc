...
[[requirements-for-allowing-s3-object-lock-to-be-enabled-on-existing-buckets]]
[[s3-object-lock-existing-buckets]]
== 允許在現有儲存桶上啟用 S3 物件鎖定的要求

啟用全域 S3 物件鎖定設定後，您可以選擇允許所有具有對應權限的 S3 租戶帳戶或 S3 使用者對現有儲存桶啟用 S3 物件鎖定。當 S3 租戶帳戶或 S3 使用者對現有儲存桶啟用 S3 物件鎖定時，根據您的 ILM 規則，可能會導致非預期的資料從雲端儲存資源池移轉回 StorageGRID。


CAUTION: 一旦意外的資料移轉開始，就無法停止。
