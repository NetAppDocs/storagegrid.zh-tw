...
---
= StorageGRID 中的 Cloud Storage Pool 使用案例
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
如果您希望將物件資料分層或備份到多個雲端平台，可以設定多個 Cloud Storage Pool 端點。ILM 規則中的篩選器可讓您指定哪些物件儲存在每個 Cloud Storage Pool 中。例如，您可能想要將某些租戶或儲存桶中的物件儲存在 Amazon S3 Glacier 中，而將其他租戶或儲存桶中的物件儲存在 Azure Blob Storage 中。或者，您可能希望在 Amazon S3 Glacier 和 Azure Blob Storage 之間移動資料。
