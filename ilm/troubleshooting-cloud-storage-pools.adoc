...
* 當您開始將 Amazon S3 儲存桶用於雲端儲存資源池後，啟用預設保留的 S3 物件鎖定時，會發生此錯誤。這是因為 PUT 操作缺少包含有效負載 Checksum 值的 HTTP 標頭，例如 `Content-MD5`。AWS 要求對已啟用 S3 物件鎖定的儲存桶執行 PUT 操作時，必須包含此標頭值。
+
若要解決此問題，請按照 link:../ilm/editing-cloud-storage-pool.html["編輯雲端儲存資源池"] 中的步驟操作，無需進行任何變更。此行動將觸發雲端儲存資源池組態的驗證，該驗證會自動偵測並更新雲端儲存資源池端點組態上的 S3 物件鎖定標誌。

* 當您開始將此儲存桶用於雲端儲存資源池後，為 StorageGRID S3 儲存桶啟用具有預設保留的 S3 物件鎖定。
+
要解決此問題：

+
** 如果您使用的是 HTTP：啟用了 S3 Object Lock 的雲端儲存資源池不受支援。請編輯端點連線，改用 HTTPS。
** 如果您使用的是 HTTPS：請依照 link:../ilm/editing-cloud-storage-pool.html["編輯雲端儲存資源池"] 中的步驟操作，無需進行任何變更。此行動將觸發雲端儲存資源池組態的驗證，該驗證會自動偵測並更新雲端儲存資源池端點組態上的 S3 物件鎖定標誌。
