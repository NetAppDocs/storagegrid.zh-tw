...
+
當物件與使用 Azure Cloud Storage Pool 作為其放置位置的 ILM 規則相符時，StorageGRID 會將物件移至 Cloud Storage Pool 指定的外部 Azure Blob 儲存 Container。

. *物件移轉至歸檔層（無法擷取的狀態）*
+
將物件移至 Azure Cloud Storage Pool 後，StorageGRID 會自動將物件轉換至 Azure Blob Storage Archive 分層。
...
+
當 StorageGRID 收到 RestoreObject 請求時，它會將物件暫時轉換到 Azure Blob 儲存體的冷分層。一旦 RestoreObject 請求中的過期日期到達，StorageGRID 就會將物件轉換回封存分層。
