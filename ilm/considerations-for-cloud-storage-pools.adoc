...
* 一般來說，雲端歸檔儲存設備（例如 Amazon S3 Glacier 或 Azure Blob 儲存設備）是儲存物件資料的一種經濟實惠的方式。然而，從雲端歸檔儲存設備擷取資料的成本相對較高。為了達到最低的整體成本，您必須考慮何時以及多久存取一次 Cloud Storage Pool 中的物件。建議僅將 Cloud Storage Pool 用於您預計存取頻率較低的內容。
...
請勿對與 Cloud Storage Pool 搭配使用的 Container 使用 Azure Blob Storage 生命週期管理。生命週期操作可能會干擾 Cloud Storage Pool 的操作。
