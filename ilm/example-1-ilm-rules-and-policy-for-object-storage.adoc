...
---
= StorageGRID 中基本物件保護和保留的 ILM 規則和原則
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:ilm_rule_two_copies_two_data_centers.png["ILM 規則 1 例如 1 ：將物件資料複製到兩個站台"]
...
|===
image:ilm_rule_ec_for_s3_bucket_finance_records.png["ILM 規則 2 範例 1 ：含貯體比對的銷毀編碼設定檔"]
...
+
image:policy_1_configured_policy.png["ILM原則（例如1）"]
